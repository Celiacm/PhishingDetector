...
DOMINIOS_GRATUITOS = ["gmail.com", "yahoo.com", "hotmail.com", "outlook.com", "live.com", "aol.com"]
...
def analyze_eml_file(eml_file):
    ...
    try:
        ...
        spf_ok, dkim_ok, dmarc_ok = extract_authentication_results(raw_bytes)


        spf_result = "✅ Válido" if spf_ok else "❌ Fallido"
        dkim_result = "✅ Válido" if dkim_ok else "❌ Fallido"
        dmarc_result = "✅ Válido" if dmarc_ok else "❌ Fallido"
...
def analizar_headers(raw_email_bytes):
    ...
    if not any("received-spf" in str(k).lower() for k in headers):
        motivos.append("Falta cabecera Received-SPF")
        score += 1

    if not any("dkim-signature" in str(k).lower() for k in headers):
        motivos.append("Falta cabecera DKIM-Signature")
        score += 1

    if not any("authentication-results" in str(k).lower() for k in headers):
        motivos.append("Falta cabecera Authentication-Results")
        score += 1

    # Cabeceras Received
    received_headers = [v for k, v in headers.items() if str(k).lower() == "received"]
    ...
    if return_path and from_header:
        _, from_email = parseaddr(str(from_header))
        _, return_email = parseaddr(str(return_path))
    ...
    for v in headers.values():
        if any(domain in str(v).lower() for domain in [".tk", ".ml", ".cf", ".ga", ".gq"]):
...
def analizar_cabeceras(raw_email_bytes):
    ...
    return {
        ...
        "dominio_gratuito": any(
            sender_email.endswith(dominio) for dominio in DOMINIOS_GRATUITOS
        ) if sender_email else False,
...
def is_phishing(email_data: dict) -> dict:
    ...
    if email_data.get("spf") is False:
        score += PESOS["auth_fail"]
        motivos.append("❌ SPF fallido")
    if email_data.get("dkim") is False:
        score += PESOS["auth_fail"]
        motivos.append("❌ DKIM fallido")
    if email_data.get("dmarc") is False:
        score += PESOS["auth_fail"]
    ...
    for enlace in email_data.get("enlaces", []):
        if enlace.get("tipo") == "sospechoso":
            ...
            motivos.append("🔗 Enlace sospechoso detectado: " + enlace['url'])
    ...
    if len(email_data.get("enlaces", [])) >= 10:
        ...
        motivos.append("⚠️ Exceso de enlaces en el correo")
    ...
    if email_data.get("html_excesivo", False):
        ...
        motivos.append("🖥️ Correo contiene HTML excesivo o sospechoso")

    # 7. Limpiar motivos de autenticación incorrectos
    motivos_limpios = []
    for m in motivos:
        if m == "❌ SPF fallido" and email_data.get("spf") is True:
            score -= PESOS["auth_fail"]
        elif m == "❌ DKIM fallido" and email_data.get("dkim") is True:
            score -= PESOS["auth_fail"]
        elif m == "❌ DMARC fallido" and email_data.get("dmarc") is True:
            score -= PESOS["auth_fail"]
        else:
            motivos_limpios.append(m)

    estado = score_a_estado(score, motivos_limpios)

    return {
        "estado": estado,
        "score": score,
        "motivos": motivos_limpios,
...
def extract_authentication_results(raw_email_bytes):
    try:
        mensaje = email.message_from_bytes(raw_email_bytes, policy=policy.default)
        auth_results = mensaje.get("Authentication-Results", "")
        ...
        # Si no hay datos fiables en Authentication-Results, haz validación manual
        if not auth_results or (not spf_pass and not dkim_pass and not dmarc_pass):
            sender = mensaje.get("From", "")
            _, sender_email = parseaddr(sender)

            if sender_email:
                spf_pass = check_spf(sender_email)
                dkim_pass = check_dkim(raw_email_bytes)
                dmarc_pass = check_dmarc(sender_email)
            else:
                spf_pass = dkim_pass = dmarc_pass = False

        return spf_pass, dkim_pass, dmarc_pass

    except Exception as e:
        print(f"[ERROR] Error extrayendo Authentication-Results: {e}")
