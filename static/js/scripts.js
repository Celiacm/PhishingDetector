...
document.addEventListener("DOMContentLoaded", estadoSistema);
  ...
  document.getElementById("emlForm").addEventListener("submit", async function(e) {
    ...
    try {
      ...
      document.getElementById("correo_id_modal").value = data.correo_id || "";
      ...
      document.querySelector("#resultadoModal .modal-body").innerHTML = `
        ...
        <div class="border rounded p-3 mb-3 bg-light">
          ...
          ${Array.isArray(data.reasons)
            ? `<ul class="list-group mb-0">${data.reasons.map(m => `<li class="list-group-item">${m}</li>`).join("")}</ul>`
            : `<p>${data.reasons}</p>`}
  ...
  function showSection(sectionId) {
    ...
    if (target) {
      ...
      // ✅ Llamar a actualizarBarraRiesgo cuando se abre el dashboard
      if (sectionId === "inicio") {
    ...
    document.querySelectorAll(".feedback-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        fetch("/feedback", {
          ...
          .then(res => {
            if (res.ok) {
              mostrarToast("✅ Feedback guardado. ¡Gracias por confirmar!");
  ...
  function mostrarToast(mensaje = "Gracias por tu feedback.") {
  ...
  document.querySelectorAll(".feedback-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const correcto = btn.dataset.correcto;
      const idCorreo = document.getElementById("correo_id_modal").value;
